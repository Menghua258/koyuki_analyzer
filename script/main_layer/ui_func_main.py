...
class MainFunc:
    ...
    def _update_all_buttons(self, styles, button_font, button_stylesheet):
        """批量更新所有功能按钮的样式和位置"""
        btn_start_x = int(self.main_ui.screen_width * styles.get('button_start_x', 0.68))
        btn_start_y = int(self.main_ui.screen_height * styles.get('button_start_y', 0.32))
        row_gap = int(self.main_ui.screen_height * styles.get('button_row_gap', 0.08))
        right_start_x = int(self.main_ui.screen_width * styles.get('right_title_x', 0.78))
        tools_start_x = int(self.main_ui.screen_width * styles.get('tools_title_x', 0.88))

        btn_font = QFont(button_font, styles.get('button_font_size', 16), QFont.Bold)

        buttons = [
            ('btn_analysis', btn_start_x, btn_start_y),
            ('btn_violin', btn_start_x, btn_start_y + row_gap),
            ('btn_bubble', btn_start_x, btn_start_y + row_gap * 2),
            ('btn_gene_bubble', btn_start_x, btn_start_y + row_gap * 3),
            ('btn_diff', btn_start_x, btn_start_y + row_gap * 4),
            ('btn_bulk', right_start_x, btn_start_y),
            ('btn_bulk_cox', right_start_x, btn_start_y + row_gap),
            ('btn_bulk_km', right_start_x, btn_start_y + row_gap * 2),
            ('btn_bulk_survival', right_start_x, btn_start_y + row_gap * 3),
            ('btn_bulk_wgcna', right_start_x, btn_start_y + row_gap * 4),
