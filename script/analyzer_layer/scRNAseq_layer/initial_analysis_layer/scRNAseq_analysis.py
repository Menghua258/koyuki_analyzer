...
class ScRNAAnalysis:
    """单细胞RNA-seq分析类 - 包含初步分析页面的核心生信分析方法"""
    ...
    def get_plot_image_path(self, source_text, plot_text):
        """根据显示类型和绘图类型返回对应的图片路径（纯数据）
        返回: (image_path, error_msg)
        """
        if self.adata is None:
            return None, "请先加载数据"
    ...
    def query_gene(self, gene_name):
        """查询基因（纯算法，不操作控件）
        返回: (success, image_path, error_msg)
        """
        if not gene_name:
