...
class WgcnaPageUI:
    def __init__(self, parent_widget, screen_width, screen_height):
        ...
        self.wgcna_page = None
        self.create_page()

    def update_background(self):
        styles = get_mod_styles()
        paths = get_mod_paths()
        bg_label = self.wgcna_page.findChild(QLabel, "wgcna_bg")
        if bg_label:
    ...
    def update_styles(self):
        ...
        title_label = self.wgcna_page.findChild(QLabel, "wgcna_title")
        if title_label:
            title_label.setStyleSheet(f"color: {styles.get('sub_mutant_color', styles.get('mutant_color', '#E91E63'))};")

        overlay = self.wgcna_page.findChild(QWidget, "wgcna_overlay")
        if overlay:
    ...
    def create_page(self):
        self.wgcna_page = QWidget(self.parent)

        styles = get_mod_styles()
        paths = get_mod_paths()
        mod_instance = global_mod_manager.get_current_mod()

        bg_label = QLabel(self.wgcna_page)
        bg_label.setObjectName("wgcna_bg")
        ...
        overlay = QWidget(self.wgcna_page)
        overlay.setObjectName("wgcna_overlay")
        ...
        self.btn_back_wgcna = create_styled_button("← 返回主页", font_size=12)
        top_layout.addWidget(self.btn_back_wgcna)

        title_label = QLabel("bulk WGCNA")
        title_label.setObjectName("wgcna_title")
        ...
        self.music_controller = MusicControllerClass(self.wgcna_page, mod_instance)
