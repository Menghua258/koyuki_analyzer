...
class BulkExprBind:
    ...
    def bind_bulk_expr_functions(self):
        ...
        if hasattr(self.bulk_expr_ui, 'btn_back_bulk_expr'):
            self.bulk_expr_ui.btn_back_bulk_expr.clicked.connect(self.go_to_home)

        # 扫描数据路径按钮
        if hasattr(self.bulk_expr_ui, 'btn_select_path'):
            bind_button_with_sound(self.bulk_expr_ui.btn_select_path, self.scan_data_path,
                                   log_widget, "扫描完成", "扫描失败")

        # 加载数据按钮
        if hasattr(self.bulk_expr_ui, 'btn_load'):
            bind_button_with_sound(self.bulk_expr_ui.btn_load, self.load_data,
