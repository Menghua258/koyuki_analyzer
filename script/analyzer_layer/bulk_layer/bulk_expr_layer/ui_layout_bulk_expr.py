...
class BulkExprPageUI:
    ...
    def create_page(self):
        ...
        self.btn_back_bulk_expr = create_styled_button("← 返回主页", font_size=12)
        ...
        # 扫描数据路径按钮 - 使用import_button样式（紫色）
        self.btn_select_path = QPushButton("扫描数据路径")
        self.btn_select_path.setFont(get_font_for_widget('button', 12))
        self.btn_select_path.setStyleSheet(get_stylesheet_for_widget('import_button'))
        left_layout.addWidget(self.btn_select_path)

        # h5ad文件选择下拉框
        self.bulk_h5ad_combo = create_styled_combo_box()
        left_layout.addWidget(self.bulk_h5ad_combo)

        # 加载数据按钮 - 使用import_button样式（紫色）
        self.btn_load = QPushButton("加载数据集")
        self.btn_load.setFont(get_font_for_widget('button', 12))
        self.btn_load.setStyleSheet(get_stylesheet_for_widget('import_button'))
