...
class BulkKmBind:
    ...
    def bind_bulk_km_functions(self):
        ...
        if hasattr(self.bulk_km_ui, 'btn_back_bulk_km'):
            self.bulk_km_ui.btn_back_bulk_km.clicked.connect(self.go_to_home)
        ...
        # 扫描数据路径按钮
        if hasattr(self.bulk_km_ui, 'btn_select_path'):
            bind_button_with_sound(self.bulk_km_ui.btn_select_path, self.scan_data_path,
                                   log_widget, "扫描完成", "扫描失败")

        # 加载数据按钮
        if hasattr(self.bulk_km_ui, 'btn_load'):
            bind_button_with_sound(self.bulk_km_ui.btn_load, self.load_data,
    ...
    def go_to_home(self):
        """返回主页"""
        if hasattr(self.parent, 'go_to_home_page'):
            self.parent.go_to_home_page()

    def go_to_km_r_page(self):
        """进入R模式页面"""
        if hasattr(self.parent, 'go_to_km_r_page'):
            self.parent.go_to_km_r_page()
