# -*- coding: utf-8 -*-
"""
bulk KM曲线界面UI布局脚本 - 只负责创建控件、规划窗口布局、摆放按钮/输入框/画布、设置样式尺寸
完全不写按钮点击、触发逻辑
继承bulk_expr_layer的样式模板
"""
...
class BulkKmPageUI:
    ...
    def update_background(self):
        ...
        paths = get_mod_paths()
        bg_label = self.bulk_km_page.findChild(QLabel, "bulk_km_bg")
        if bg_label:
    ...
    def update_styles(self):
        ...
        title_label = self.bulk_km_page.findChild(QLabel, "bulk_km_title")
        if title_label:
        ...
        for child in self.bulk_km_page.findChildren(QPushButton):
        ...
        for child in self.bulk_km_page.findChildren(QComboBox):
            child.setStyleSheet(combo_style)

        line_edit_style = get_stylesheet_for_widget('line_edit')
        for child in self.bulk_km_page.findChildren(QLineEdit):
            child.setStyleSheet(line_edit_style)

        text_edit_style = get_stylesheet_for_widget('text_edit')
        for child in self.bulk_km_page.findChildren(QTextEdit):
            child.setStyleSheet(text_edit_style)

        label_style = get_stylesheet_for_widget('label')
        for child in self.bulk_km_page.findChildren(QLabel):
            if child.objectName() != "bulk_km_title" and not child.objectName().startswith("styled_image_label"):
                child.setStyleSheet(label_style)

        checkbox_style = get_stylesheet_for_widget('checkbox')
        for child in self.bulk_km_page.findChildren(QCheckBox):
    ...
    def create_page(self):
        ...
        # 背景图
        bg_label = QLabel(self.bulk_km_page)
        bg_label.setObjectName("bulk_km_bg")
        ...
        # 覆盖层
        overlay = QWidget(self.bulk_km_page)
        overlay.setObjectName("bulk_km_overlay")
        ...
        self.btn_back_bulk_km = create_styled_button("← 返回主页", font_size=12)
        top_layout.addWidget(self.btn_back_bulk_km)

        title_label = QLabel("bulk KM曲线")
        title_label.setObjectName("bulk_km_title")
        ...
        self.music_controller = MusicControllerClass(self.bulk_km_page, mod_instance)
        ...
        self.bulk_km_r_mode_btn = create_styled_button("进入R模式", font_size=10, variant='sub')
        self.bulk_km_r_mode_btn.setMinimumWidth(320)
        status_layout.addWidget(self.bulk_km_r_mode_btn)
        ...
        # 扫描数据路径按钮
        self.btn_select_path = create_styled_button("扫描数据路径", font_size=12, variant='import')
        left_layout.addWidget(self.btn_select_path)

        # h5ad文件选择下拉框
        self.bulk_km_h5ad_combo = create_styled_combo_box()
        left_layout.addWidget(self.bulk_km_h5ad_combo)

        # 加载数据按钮
        self.btn_load = create_styled_button("加载数据集", font_size=12, variant='import')
        ...
        self.bulk_km_pval_mode_combo.setCurrentIndex(2)
