...
from PyQt5.QtWidgets import QFileDialog, QMessageBox, QApplication, QListWidgetItem

from script.analyzer_layer.bulk_layer.bulk_km_layer.bulk_km_r_layer.bulk_km_r_analysis import get_bulk_km_r_analysis
from script.analyzer_layer.bulk_layer.bulk_km_layer.bulk_km_r_layer.ui_func_bulk_km_r import BulkKmRFunc
...
class BulkKmRBind:
    ...
    def sync_data_from_km(self):
        """从KM页面同步数据"""
        try:
            # 检查父对象是否有bulk_km_bind
            if not hasattr(self.parent, 'bulk_km_bind'):
                self.func.log("错误: 无法从KM模式同步数据（父对象没有bulk_km_bind）")
                return
            
            km_bind = self.parent.bulk_km_bind
            if km_bind is None:
                self.func.log("错误: KM模式数据未加载")
                return
    ...
    def on_back_clicked(self):
        """返回KM模式"""
        if hasattr(self.parent, 'show_bulk_km_page'):
            self.parent.show_bulk_km_page()
    ...
    def _draw_combined_km_plot(self, all_km_data, time_label='Time (days)',
        ...
        if fmt == 'png':
            ...
            return self._combine_pdf_vectors(all_km_data, n_cols, n_rows, export_width, export_height)
        elif fmt == 'svg':
            return self._combine_svg_vectors(all_km_data, n_cols, n_rows, export_width, export_height)
        else:
            return None

    def _combine_pdf_vectors(self, all_km_data, n_cols, n_rows, export_width, export_height):
        """使用PyPDF2将多个PDF组合成网格布局页面"""
        ...
        if n_plots == 1:
            output_path = os.path.join(self.dataset_output_dir or ".", f"{self.dataset_name}_combined_km.pdf")
            import shutil
            shutil.copy(pdf_files[0], output_path)
            return output_path

        first_reader = PdfReader(pdf_files[0])
        first_page = first_reader.pages[0]
        page_width = float(first_page.mediabox.width)
        page_height = float(first_page.mediabox.height)

        total_width = page_width * n_cols
        total_height = page_height * n_rows

        writer = PdfWriter()
        combined_page = writer.add_blank_page(width=total_width, height=total_height)
        ...
        for i, pdf_file in enumerate(pdf_files):
            ...
            col = i % n_cols
            x = col * page_width
            y = total_height - (row + 1) * page_height

            src_page.add_transformation(Transformation().translate(x, y))
            src_page.mediabox.upper_right = (total_width, total_height)
            src_page.mediabox.lower_left = (0, 0)
            combined_page.merge_page(src_page)
